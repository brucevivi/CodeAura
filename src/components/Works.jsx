...
const ProjectCard = ({ name, description, tags, image, source_code_link, web_link }) => {
  return (
      ...
      >
        <div className="relative w-full h-[230px]">
          ...
          <div className="absolute inset-0 flex justify-end m-3 card-img_hover">
            ...
            >
              <img src={github} alt="source code" className="w-8 h-8 object-contain bg-black" />
            </div>
          </div>
        </div>
        <div className="mt-5">
          <h3 className="text-white font-bold text-[24px] cursor-pointer"  onClick={() => window.open(web_link, "_blank")}
>{name}</h3>
          <p className="mt-2 text-secondary text-[14px] cursor-pointer" onClick={() => window.open(web_link, "_blank")}>{description}</p>
        </div>
        <div className="mt-4 flex flex-wrap gap-2 cursor-pointer"  onClick={() => window.open(web_link, "_blank")}>
          {tags.map((tag) => (
